...
import {BdtPriRestri} from '../../bie-management/bie-edit/domain/bie-edit-node';
...
import {BccpFlatNode, BdtFlatNode, CcFlatNode, CcFlatNodeFlattener, VSCcTreeDataSource} from '../domain/cc-flat-tree';
import {CcNodeService} from '../domain/core-component-node.service';
import {
  CcAccNodeDetail,
  CcAsccpNodeDetail,
  CcBccpNodeDetail, CcBdtNodeDetail, CcBdtPriRestri,
  CcBdtScNodeDetail, CcBdtScPriRestri,
  CcNodeDetail,
  CcRevisionResponse, CcXbt,
  Comment,
  EntityType,
  EntityTypes,
  OagisComponentType,
...
export class BdtDetailComponent implements OnInit {
  title: string;
  ...
  selectedBdtPriRestri: CcBdtPriRestri;
  selectedXbt: CcXbt;

  selectedBdtScPriRestri: CcBdtScPriRestri;
  ...
  ngOnInit() {
    ...
    this.route.paramMap.pipe(
      ...
      // subscribe an event
      this.stompService.watch('/topic/bdt/' + this.manifestId).subscribe((message: Message) => {
        const data = JSON.parse(message.body);
        if (data.properties.actor !== this.currentUser) {
          let noti;
          if (data.action === 'UpdateDetail') {
            noti = 'Bdt updated by ' + data.properties.actor;
      ...
      const flattener = new CcFlatNodeFlattener(ccGraph, 'BDT', this.manifestId);
      setTimeout(() => {
        ...
        this.rootNode = nodes[0] as BdtFlatNode;
  ...
  isEditable() {
    if (!this.isBdtDetail() || this.selectedNode.manifestId !== this.manifestId) {
      return false;
    }
  ...
  onClick(node: CcFlatNode, $event?: MouseEvent) {
    ...
    this.dataSource.loadDetail(node, (detail: CcNodeDetail) => {
      if (node.type == 'BDT') {
        const bdtPriRestriList = (detail as CcBdtNodeDetail).bdtPriRestriList.map(row => new CcBdtPriRestri(row));
        (node.detail as CcBdtNodeDetail).bdtPriRestriList = bdtPriRestriList;
        this.selectedBdtPriRestri = (detail as CcBdtNodeDetail).bdtPriRestriList.filter(e => e.default)[0];
        this.selectedXbt = this.selectedBdtPriRestri.xbtList.filter(e => e.default)[0];

      } else if (node.type == 'BDT_SC'){
        const bdtScPriRestriList = (detail as CcBdtScNodeDetail).bdtScPriRestriList.map(row => new CcBdtScPriRestri(row));
        (node.detail as CcBdtScNodeDetail).bdtScPriRestriList = bdtScPriRestriList;
        this.selectedBdtScPriRestri = (detail as CcBdtScNodeDetail).bdtScPriRestriList.filter(e => e.default)[0];
        this.selectedXbt = this.selectedBdtScPriRestri.xbtList.filter(e => e.default)[0];
      }
  ...
  isBdtDetail(node?: CcFlatNode): boolean {
    ...
    return (node !== undefined) && (node.type.toUpperCase() === 'BDT');
  }

  asBdtDetail(node?: CcFlatNode): CcBdtNodeDetail {
    if (!node) {
      node = this.selectedNode;
    }
    return node.detail as CcBdtNodeDetail;
  }
