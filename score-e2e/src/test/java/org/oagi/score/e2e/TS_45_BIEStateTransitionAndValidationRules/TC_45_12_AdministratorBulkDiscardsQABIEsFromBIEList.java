...
public class TC_45_12_AdministratorBulkDiscardsQABIEsFromBIEList extends TS45Base {

    @Test
    @DisplayName("TC_45_12_1_and_TC_45_12_2_and_TC_45_12_3")
    public void administrator_bulk_discards_qa_bies_from_bie_list() {
        ...
        ViewEditBIEPage viewEditBIEPage = adminHomePage.getBIEMenu().openViewEditBIESubMenu();
        viewEditBIEPage.showAdvancedSearchPanel();
        viewEditBIEPage.setBranch(graph.primaryBaseBIE.getReleaseNumber());
        viewEditBIEPage.setOwner(graph.endUser.getLoginId());
        viewEditBIEPage.hitSearchButton();

        WebElement baseRow = visibilityOfElementLocated(getDriver(), By.xpath(
                "//tbody/tr[.//td[contains(@class, 'mat-column-version')]//*[contains(text(), \"" +
                        graph.primaryBaseBIE.getVersion() + "\")]]"));
        click(viewEditBIEPage.getColumnByName(baseRow, "select"));
        WebElement classificationRow = visibilityOfElementLocated(getDriver(), By.xpath(
                "//tbody/tr[.//td[contains(@class, 'mat-column-version')]//*[contains(text(), \"" +
                        graph.sharedReusableClassificationBIE.getVersion() + "\")]]"));
        click(viewEditBIEPage.getColumnByName(classificationRow, "select"));
        click(viewEditBIEPage.getDiscardButton(true));

        assertTrue(!getDependencyDialogDiscardButton().isEnabled());
        assertDependencySummaryContains(VALIDATION_SUMMARY_MULTI_DISCARD);
        ...
        assertBIEDeleted(graph.sharedReusableClassificationBIE);
