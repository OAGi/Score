...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`dt` (
  ...
  `type` INT NOT NULL COMMENT 'List value: 0 = CDT, 1 = BDT.',
  ...
  `previous_version_dt_id` INT NULL,
  ...
  `based_dt_id` INT NULL COMMENT 'Foreign key pointing to itself. This column must be blank when the DT_Type is CDT. This column must not be blank when the DT_Type is BDT.',
  ...
  `state` INT NULL COMMENT '1 = Editing, 2 = Candidate, 3 = Published. This the revision life cycle state of the entity.\n\nState change can\'t be undone. But the history record can still keep the records of when the state was changed.',
  `created_by` INT NOT NULL,
  `owner_user_id` INT NOT NULL COMMENT 'This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn\'t rollback the ownership. ',
  `last_updated_by` INT NOT NULL,
  `creation_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `revision_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Number is an incremental integer. It tracks changes in each component. If a change is made to a component after it has been published, the component receives a new revision number. Revision number can be 0, 1, 2, and so on. A record with zero revision number reflects the current record of the component (the identity of a component in this case is its GUID or the primary key).',
  `revision_tracking_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Tracking_Number supports the ability to undo changes during a revision (life cycle of a revision is from the component\'s Editing state to Published state). Once the component has transitioned into the Published state for its particular revision, all revision tracking records are deleted except the latest one. Revision_Tracking_Number can be 0, 1, 2, and so on. The zero value is assign to the record with Revision_Number = 0 as a default.',
  `revision_action` TINYINT NULL DEFAULT 1 COMMENT 'This indicates the action associated with the record. The action can be 1 = insert, 2 = update, and 3 = delete. This column is null for the current record.',
  `release_id` INT NULL COMMENT 'Release_ID is an incremental integer. It is an unformatted counter part of the Release_Number in the Release table. Release_ID can be 1, 2, 3, and so on. Release ID indicates the release point when a particular component revision is released. A component revision is only released once and assumed to be included in the subsequent releases unless it has been deleted (as indicated by the Revision_Action column).\n\nNot all component revisions have an associated Release_ID because some revisions may never be released.\n\nUnpublished components cannot be released.\n\nThis column is null for the current record.',
  `current_bdt_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`xbt` (
  `xbt_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Unique ID of XML Schema built-in types.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`cdt_pri` (
  `cdt_pri_id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`cdt_awd_pri` (
  `cdt_awd_pri_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Primary key.',
  `cdt_id` INT NOT NULL COMMENT 'Foreign key from the dt table corresponding to the CDT being recorded.',
  `cdt_pri_id` INT NOT NULL COMMENT 'Foreign key from the cdt_pri table corresponding to the Allowed Primitive column in each of the CDT Content Component section/table in CCTS DTC3',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`cdt_awd_pri_xps_type_map` (
  `cdt_awd_pri_xps_type_map_id` INT NOT NULL AUTO_INCREMENT,
  `cdt_awd_pri_id` INT NOT NULL,
  `xbt_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`dt_sc` (
  `dt_sc_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Revision is not tracked at the supplementary component. It is considered intrinsic part of the DT. In other words, when a new revision of a DT is created a new set of supplementary components is created along with it. ',
  `guid` VARCHAR(41) NOT NULL,
  `property_term` VARCHAR(45) NULL,
  `representation_term` VARCHAR(45) NULL,
  `definition` TEXT NULL,
  `owner_dt_id` INT NOT NULL COMMENT 'Foreigned key to the Core_Data_Type table indicating the data type to which this supplementary component belongs.',
  `min_cardinality` INT NOT NULL DEFAULT 0,
  `max_cardinality` INT NULL,
  `based_dt_sc_id` INT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`cdt_sc_awd_pri` (
  `cdt_sc_awd_pri_id` INT NOT NULL AUTO_INCREMENT,
  `cdt_sc_id` INT NOT NULL,
  `cdt_pri_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`namespace` (
  ...
  `owner_user_id` INT NOT NULL COMMENT 'The user who can update or delete the record.',
  `created_by` INT NOT NULL COMMENT 'The user who created the namespace.',
  `last_updated_by` INT NOT NULL COMMENT 'The use who last updated the record.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bccp` (
  `bccp_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
  ...
  `bdt_id` INT NOT NULL COMMENT 'Only DT_ID which DT_Type is BDT can be used.',
  `den` TEXT NOT NULL,
  `definition` TEXT NULL,
  ...
  `namespace_id` INT NULL COMMENT 'Foreign key to the Namespace table. This is the namespace, to which the entity belongs.',
  `is_deprecated` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Indicates whether the CC is deprecated and should not be reused (i.e., no new reference to this record should be created).',
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the entity.\n\nThis column never change between the history and the current record. The history record should have the same value as that of its current record.',
  `owner_user_id` INT NOT NULL,
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who has updated the record. \n\nIn the history record, this should always be the user who is editing the entity.',
  ...
  `state` INT NOT NULL COMMENT '1 = Editing, 2 = Candidate, 3 = Published. This the revision life cycle state of the ACC.\n\nState change can\'t be undone. But the history record can still keep the records of when the state was changed.',
  `revision_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Number is an incremental integer. It tracks changes in each component. If a change is made to a component after it has been published, the component receives a new revision number. Revision number can be 0, 1, 2, and so on. A record with zero revision number reflects the current record of the component (the identity of a component in this case is its GUID or the primary key).',
  `revision_tracking_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Tracking_Number supports the ability to undo changes during a revision (life cycle of a revision is from the component\'s Editing state to Published state). Once the component has transitioned into the Published state for its particular revision, all revision tracking records are deleted except the latest one. Revision_Tracking_Number can be 0, 1, 2, and so on. The zero value is assign to the record with Revision_Number = 0 as a default.',
  `revision_action` INT NULL DEFAULT 1 COMMENT 'This indicates the action associated with the record. The action can be 1 = insert, 2 = update, and 3 = delete. This column is null for the current record.',
  `release_id` INT NULL COMMENT 'Release_ID is an incremental integer. It is an unformatted counter part of the Release_Number in the Release table. Release_ID can be 1, 2, 3, and so on. Release ID indicates the release point when a particular component revision is released. A component revision is only released once and assumed to be included in the subsequent releases unless it has been deleted (as indicated by the Revision_Action column).\n\nNot all component revisions have an associated Release_ID because some revisions may never be released.\n\nUnpublished components cannot be released.\n\nThis column is null for the current record.',
  `current_bccp_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
  PRIMARY KEY (`bccp_id`),
  CONSTRAINT `BDT`
  FOREIGN KEY (`bdt_id`)
  REFERENCES `oagsrt_revision`.`dt` (`dt_id`)
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`release` (
  `release_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Release_ID must be an incremental integer. Release_ID that is more than another Release_ID is interpreted to be released later than the other.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`acc` (
  `acc_id` INT NOT NULL AUTO_INCREMENT COMMENT 'A internal, primary database key of an ACC.',
  ...
  `based_acc_id` INT NULL COMMENT 'Based_ACC_ID is a foreign key to the ACC table itself. It represents the ACC that is qualified by this ACC. In general CCS sense, a qualification can by a content extension or restriction, but the current scope supports only extension.\n\nFor history records of an ACC, this column always points to the current record of an ACC.',
  `object_class_qualifier` TEXT NULL,
  `oagis_component_type` INT NULL COMMENT 'The value can be 0 = Base, 1 = Semantics, 2 = Extension, 3 = Semantic Group, 4 = User Extension Group. Generally, Bsae is assigned when the Object_Class_Term contains \"Base\" at the end. Extension is assigned with the Object_Class_Term contains \"Extension\" at the end. Semantic Group is assigned when an ACC is imported from an XSD Group. Other cases are assigned Semantics.',
  ...
  `namespace_id` INT NULL COMMENT 'Foreign key to the Namespace table. This is the namespace to which the entity belongs. This namespace column is only used in the case the component is a user\'s component.',
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the entity.\n\nThis column never change between the history and the current record. The history record should have the same value as that of its current record.',
  `owner_user_id` INT NOT NULL COMMENT 'This is the user who owns the entity, is allowed to edit the entity, and who can transfer the ownership to another user.\n\nThe ownership can change throughout the history, but undoing shouldn\'t rollback the ownership. ',
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who updated the record. \n\nIn the history record, this should always be the user who is editing the entity.',
  ...
  `current_acc_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
  `is_deprecated` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Indicates whether the CC is deprecated and should not be reused (i.e., no new reference to this record should be created).',
  PRIMARY KEY (`acc_id`),
  CONSTRAINT `Base_ACC`
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bcc` (
  ...
  `cardinality_max` INT NULL COMMENT '-1 means unbounded.',
  `to_bccp_id` INT NOT NULL,
  `from_acc_id` INT NOT NULL,
  `seq_key` INT NULL COMMENT 'This indicates the order of the associations among other siblings. The valid values are positive integer. The Sequencing_Key at the CC side is localized. In other words, if an ACC is based on another ACC, Sequencing_Key of ASCCs or BCCs of the former ACC starts at 1 again. The Sequencing_Key in the case of Entity_Type is attribute is always zero.',
  `entity_type` INT NULL COMMENT 'This is a code list: 0 = attribute and 1 = element. An expression generator may or may not use this information. This column is necessary because some of the BCCs are xsd:attribute and some are xsd:element in the legacy OAGIS. ',
  `den` TEXT NOT NULL,
  `definition` TEXT NULL,
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the entity.\n\nThis column never change between the history and the current record. The history record should have the same value as that of its current record.',
  `owner_user_id` INT NOT NULL,
  ...
  `release_id` INT NULL COMMENT 'Release_ID is an incremental integer. It is an unformatted counter part of the Release_Number in the Release table. Release_ID can be 1, 2, 3, and so on. Release ID indicates the release point when a particular component revision is released. A component revision is only released once and assumed to be included in the subsequent releases unless it has been deleted (as indicated by the Revision_Action column).\n\nNot all component revisions have an associated Release_ID because some revisions may never be released.\n\nUnpublished components cannot be released.\n\nThis column is null for the current record.',
  `current_bcc_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`biz_ctx` (
  `biz_ctx_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`abie` (
  `abie_id` INT NOT NULL AUTO_INCREMENT COMMENT 'A internal, primary database key of an ABIE.',
  ...
  `based_acc_id` INT NOT NULL COMMENT 'A foreign key to the ACC table refering to the ACC, on which the business context has been applied to derive this ABIE.',
  ...
  `biz_ctx_id` INT NOT NULL COMMENT 'A foreign key to the Business_Context table. This column stores the business context assigned to an ABIE.',
  ...
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the ABIE. The creator of the ABIE is also its owner by default. ABIEs created as children of another ABIE have the same Created_By_User_ID.',
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASBIE record. This may be the user who is in the same group as the creator.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bbiep` (
  `bbiep_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
  `based_bccp_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`code_list` (
  `code_list_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) CHARACTER SET 'utf8' NOT NULL,
  ...
  `based_code_list_id` INT NULL COMMENT 'This indicates that this code list is based on another code list - restriction and extension are allowed.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`agency_id_list_value` (
  `agency_id_list_value_id` INT NOT NULL AUTO_INCREMENT,
  `value` TEXT NOT NULL,
  `name` TEXT NULL DEFAULT NULL,
  `definition` TEXT NULL,
  `owner_list_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`agency_id_list` (
  `agency_id_list_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) CHARACTER SET 'utf8' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bdt_pri_restri` (
  `bdt_pri_restri_id` INT NOT NULL AUTO_INCREMENT,
  `bdt_id` INT NOT NULL,
  `cdt_awd_pri_xps_type_map_id` INT NULL COMMENT 'Both CDT_Primitive_Expression_Type_Map_ID and Code_List_ID cannot be blank at the same time.',
  ...
  `agency_id_list_id` INT NULL COMMENT 'This is a foreign key to the agency_id_list table. It is used in the case that the BDT content can be restricted to an agency ID list.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bbie` (
  `bbie_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
  `based_bcc_id` INT NOT NULL,
  `from_abie_id` INT NOT NULL COMMENT 'Assoc_From_ABIE must be based on the Assoc_From_ACC in the Based_BCC.',
  `to_bbiep_id` INT NOT NULL,
  `bdt_pri_restri_id` INT NULL,
  `code_list_id` INT NULL,
  `cardinality_min` INT NOT NULL,
  `cardinality_max` INT NULL COMMENT 'Unspecified = unbounded',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`cdt_sc_awd_pri_xps_type_map` (
  `cdt_sc_awd_pri_xps_type_map_id` INT NOT NULL AUTO_INCREMENT,
  `cdt_sc_awd_pri` INT NOT NULL,
  `xbt_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bdt_sc_pri_restri` (
  `bdt_sc_pri_restri_id` INT NOT NULL AUTO_INCREMENT,
  `bdt_sc_id` INT NOT NULL,
  `cdt_sc_awd_pri_xps_type_map_id` INT NULL COMMENT 'This column is used when the BDT is derived from the CDT.',
  `code_list_id` INT NULL COMMENT 'Foreign key to identify the code list.',
  `is_default` TINYINT(1) NOT NULL,
  `agency_id_list_id` INT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`asccp` (
  `asccp_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
  ...
  `role_of_acc_id` INT NULL COMMENT 'The ACC from which this ASCCP is created (ASCCP applies role to the ACC).',
  `den` TEXT NULL,
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the entity. \n\nThis column never change between the history and the current record. The history record should have the same value as that of its current record.',
  `owner_user_id` INT NOT NULL,
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who has updated the record. \n\nIn the history record, this should always be the user who is editing the entity.',
  ...
  `revision_action` TINYINT NULL DEFAULT 1 COMMENT 'This indicates the action associated with the record. The action can be 1 = insert, 2 = update, and 3 = delete. This column is null for the current record.',
  `release_id` INT NULL COMMENT 'Release_ID is an incremental integer. It is an unformatted counter part of the Release_Number in the Release table. Release_ID can be 1, 2, 3, and so on. Release ID indicates the release point when a particular component revision is released. A component revision is only released once and assumed to be included in the subsequent releases unless it has been deleted (as indicated by the Revision_Action column).\n\nNot all component revisions have an associated Release_ID because some revisions may never be released.\n\nUnpublished components cannot be released.\n\nThis column is null for the current record.',
  `current_asccp_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`asbiep` (
  `asbiep_id` INT NOT NULL AUTO_INCREMENT COMMENT 'A internal, primary database key of an ASBIEP.',
  ...
  `based_asccp_id` INT NOT NULL COMMENT 'A foreign key point to the ASCCP record. It is the ASCCP which the ASBIEP contextualizes.',
  `role_of_abie_id` INT NOT NULL COMMENT 'A foreign key pointing to the ABIE record. It is the ABIE which the property term in the based ASCCP qualifies. Note that the ABIE has to be derived from the ACC used by the based ASCCP.',
  ...
  `remark` VARCHAR(225) NULL COMMENT 'This column allows the user to specify very context-specific usage of the BIE. It is different from the Definition column in that the Definition column is a description conveying the meaning of the associated concept. Remarks may be a very implementation specific instruction or others. For example, BOM BOD, as an ACC, is a generic BOM structure. In a particular context, a BOM ABIE can be a Super BOM. Explanation of the Super BOM concept should be captured in the Definition of the ABIE. A remark about that ABIE may be \"Type of BOM should be recognized in the BOM/typeCode.\"',
  `biz_term` VARCHAR(225) NULL COMMENT 'To indicate what the BIE is called in a particular business context. With this current design, only one business term is allowed per business context.',
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the ASBIEP. The creator of the ASBIEP is also its owner by default. ASBIEPs created as children of another ABIE have the same Created_By_User_ID.',
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASBIEP record. This may be the user who is in the same group as the creator.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`ascc` (
  `ascc_id` INT NOT NULL AUTO_INCREMENT COMMENT 'A internal, primary database key of an ASCC.',
  ...
  `cardinality_min` INT NOT NULL COMMENT 'Minimum cardinality of the Assoc_To_ASCCP_ID. The valid values are non-negative integer.',
  `cardinality_max` INT NOT NULL COMMENT 'Maximum cardinality of the Assoc_To_ASCCP_ID. The valid values are integer -1 and up. Specifically, -1 means unbounded. 0 means prohibited or not to use.',
  `seq_key` INT NOT NULL COMMENT 'This indicates the order of the associations among other siblings. The valid values are positive integer. The Sequencing_Key at the CC side is localized. In other words, if an ACC is based on another ACC, Sequencing_Key of ASCCs or BCCs of the former ACC starts at 1 again. ',
  `from_acc_id` INT NOT NULL COMMENT 'Assoc_From_ACC_ID is a foreign key pointing to an ACC record. It is basically pointing to a parent data element (type) of the Assoc_To_ASCCP_ID. \n\nNote that for the ASCC history records, this column always points to the ACC_ID of the current record of an ACC.',
  `to_asccp_id` INT NOT NULL COMMENT 'Assoc_To_ASCCP_ID is a foreign key to an ASCCP table record. It is basically pointing to a child data element of the Assoc_From_ACC_ID. \n\nNote that for the ASCC history records, this column always points to the ASCCP_ID of the current record of an ASCCP.',
  ...
  `last_update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'The timestamp when the record was last updated.\n\nThe value of this column in the latest history record should be the same as that of the current record. This column keeps the record of when the revision has occurred.',
  `state` INT NOT NULL COMMENT '1 = Editing, 2 = Candidate, 3 = Published. This the revision life cycle state of the entity.\n\nState change can\'t be undone. But the history record can still keep the records of when the state was changed.',
  `revision_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Number is an incremental integer. It tracks changes in each component. If a change is made to a component after it has been published, the component receives a new revision number. Revision number can be 0, 1, 2, and so on. A record with zero revision number reflects the current record of the component (the identity of a component in this case is its GUID or the primary key).',
  `revision_tracking_num` INT NOT NULL DEFAULT 0 COMMENT 'Revision_Tracking_Number supports the ability to undo changes during a revision (life cycle of a revision is from the component\'s Editing state to Published state). Once the component has transitioned into the Published state for its particular revision, all revision tracking records are deleted except the latest one. Revision_Tracking_Number can be 0, 1, 2, and so on. The zero value is assign to the record with Revision_Number = 0 as a default.',
  `revision_action` TINYINT NULL DEFAULT 1 COMMENT 'This indicates the action associated with the record. The action can be 1 = insert, 2 = update, and 3 = delete. This column is null for the current record.',
  `release_id` INT NULL COMMENT 'Release_ID is an incremental integer. It is an unformatted counter part of the Release_Number in the Release table. Release_ID can be 1, 2, 3, and so on. Release ID indicates the release point when a particular component revision is released. A component revision is only released once and assumed to be included in the subsequent releases unless it has been deleted (as indicated by the Revision_Action column).\n\nNot all component revisions have an associated Release_ID because some revisions may never be released.\n\nUnpublished components cannot be released.\n\nThis column is null for the current record.',
  `current_ascc_id` INT NULL COMMENT 'This is a self-foreign-key. It points from a revised record to the current record. The current record is denoted by the the record whose Revision_Number is 0. Revised records (a.k.a. history records) and their current record must have the same GUID.\n\nIt is noted that although this is a foreign key by definition, we don\'t specify a foreign key in the data model. This is because when an entity is deleted the current record won\'t exist anymore.\n\nThe value of this column for the current record should be left NULL.',
  PRIMARY KEY (`ascc_id`),
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`asbie` (
  `asbie_id` INT NOT NULL AUTO_INCREMENT COMMENT 'A internal, primary database key of an ASBIE.',
  ...
  `from_abie_id` INT NOT NULL COMMENT 'Assoc_From_ABIE_ID is a foreign key pointing to the ABIE table. Assoc_From_ABIE_ID is basically  a parent data element (type) of the Assoc_To_ASBIEP_ID. Assoc_From_ABIE_ID must be based on the Assoc_From_ACC_ID in the Based_ASCC except when the Assoc_From_ACC_ID refers to an ACC Semantic Group.',
  `to_asbiep_id` INT NOT NULL COMMENT 'Assoc_To_ASBIEP_ID is a foreign key to the ASBIEP table. Assoc_To_ASBIEP_ID is basically a child data element of the Assoc_From_ABIE_ID. Assoc_To_ASBIEP_ID must be based on the Role_of_ACC_ID in the Based_ASCC.',
  `based_ascc` INT NOT NULL COMMENT 'The Based_ASCC column refers to the ASCC record, which this ASBIE contextualizes.',
  ...
  `cardinality_min` INT NOT NULL COMMENT 'Minimum cardinality of the Assoc_To_ASBIEP_ID. The valid values are non-negative integer.',
  `cardinality_max` INT NOT NULL COMMENT 'Maximum cardinality of the Assoc_To_ASBIEP_ID. The valid values are integer -1 and up. Specifically, -1 means unbounded. 0 means prohibited or not to use.',
  `is_nillable` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Indicate whether the Assoc_To_ASBIEP is nillable.',
  `remark` VARCHAR(225) NULL COMMENT 'This column allows the user to specify very context-specific usage of the BIE. It is different from the Definition column in that the Definition column is a description conveying the meaning of the associated concept. Remarks may be a very implementation specific instruction or others. For example, BOM BOD, as an ACC, is a generic BOM structure. In a particular context, a BOM ABIE can be a Super BOM. Explanation of the Super BOM concept should be captured in the Definition of the ABIE. A remark about that ABIE may be \"Type of BOM should be recognized in the BOM/typeCode.\"',
  `created_by` INT NOT NULL COMMENT 'A foreign key referring to the user who creates the ASBIE. The creator of the ASBIE is also its owner by default. ASBIEs created as children of another ABIE have the same Created_By_User_ID.',
  `last_updated_by` INT NOT NULL COMMENT 'A foreign key referring to the last user who has updated the ASBIE record. This may be the user who is in the same group as the creator.',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`code_list_value` (
  `code_list_value_id` INT NOT NULL AUTO_INCREMENT,
  `code_list_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`ctx_category` (
  `ctx_category_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`classification_ctx_scheme` (
  `classification_ctx_scheme_id` INT NOT NULL AUTO_INCREMENT COMMENT 'External identification of the scheme.',
  `guid` VARCHAR(41) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`ctx_scheme_value` (
  `ctx_scheme_value_id` INT NOT NULL AUTO_INCREMENT,
  `guid` VARCHAR(41) NOT NULL,
  `value` VARCHAR(45) NOT NULL,
  `meaning` TEXT NULL,
  `owner_ctx_scheme_id` INT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`biz_ctx_value` (
  `biz_ctx_value_id` INT NOT NULL AUTO_INCREMENT,
  `biz_ctx_id` INT NOT NULL,
  `ctx_scheme_value_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bbie_sc` (
  `bbie_sc_id` INT NOT NULL AUTO_INCREMENT,
  `bbie_id` INT NOT NULL,
  `dt_sc_id` INT NOT NULL COMMENT 'This should correspond to the DT_SC of the BDT of the based BCC and BCCP.',
  `dt_sc_pri_restri_id` INT NULL COMMENT 'This must be one of the allowed primitive/code list as specified in the corresponding SC of the based BCC of the BBIE.',
  `code_list_id` INT NULL,
  `agency_id_list_id` INT NULL,
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`bie_user_ext_revision` (
  `bie_user_ext_revision_id` INT NOT NULL COMMENT 'Auto-generate primary key of the table.',
  `top_level_abie_id` INT NOT NULL COMMENT 'This points to an ABIE record which is a top-level ABIE. The record must have the isTop_Level flag true.',
  `ext_abie_id` INT NULL COMMENT 'This points to an ABIE record corresponding to the Extension_ACC_ID record. For example, this column can point to the ApplicationAreaExtension ABIE which is based on the ApplicationAreaExtension ACC (referred by the Extension_ACC_ID column). This column can be Null only when the extension is the AllExtension because there is no corresponding ABIE for the AllExtension ACC.',
  `ext_acc_id` INT NOT NULL COMMENT 'This points to an extension ACC on which the ABIE indicated by the Extension_ABIE_ID column is based. E.g. It may point to an ApplicationAreaExtension ACC, AllExtension ACC, ActualLedgerExtension ACC, etc. It should be noted that an ACC record pointed to must have the OAGIS_Component_Type = 2 (Extension).',
  `user_ext_acc_id` INT NOT NULL COMMENT 'This column points to the specific revision of a User Extension ACC (this is an ACC whose OAGIS_Component_Type = 4) currently used by the ABIE as indicated by the Extension_ABIE_ID or the by the Top_Level_ABIE_ID (in case of the AllExtension). ',
...
CREATE TABLE IF NOT EXISTS `oagsrt_revision`.`blob_content` (
  `blob_content_id` INT NOT NULL COMMENT 'Primary key.',
  `content` BLOB NOT NULL COMMENT 'The Blob content of the schema file.',
  `release_id` INT NOT NULL COMMENT 'The release to which this file belongs/published.',
