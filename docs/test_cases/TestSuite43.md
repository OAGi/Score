...
On the Create OpenAPI Document page, the `OpenAPI Version` field defaults to `3.1` when the page is opened, so a newly created document starts as an OpenAPI 3.1 document unless the end user changes it. The `OpenAPI Version` field exposes only the minor family `3.0` and `3.1` (Issues #1610, #1760).
...
21. Verify that the `OpenAPI Version` field defaults to `3.1`. (Assertion [#11](#test-assertion-43111))
...
Pre-condition: An end-user account that can access the BIE menu exists, and at least one OpenAPI Document already has at least two BIE operations assigned and uses OpenAPI Version 3.0, so that Generate is enabled and a per-operation Security cell is editable.
...
### Test Step Pre-condition:
1. An end-user account that can access the BIE menu is available in connectCenter.
2. At least one OpenAPI Document exists with OpenAPI Version 3.0 and at least two BIE operations assigned, so that Generate is enabled and the Security cell is editable for operations.
...
Pre-condition: An end-user account that can access the BIE menu exists, and the environment allows downloading the generated OpenAPI document file (YAML) for inspection. A new OpenAPI Document defaults to OpenAPI Version `3.1`, and the `OpenAPI Version` selector on the Edit OpenAPI Document page offers both `3.0` and `3.1` (the minor family only). At `Generate` time the backend pins the canonical patch for the selected family into the generated document's root `openapi` field — the `3.1` family emits `openapi: 3.1.2` and the `3.0` family emits `openapi: 3.0.4` — so the generated field carries the pinned patch even though the selector shows only the minor family (Issue #1760).

### Test Assertion:

#### Test Assertion #43.9.1
An OpenAPI Document whose OpenAPI Version is `3.1`, after a BIE is assigned and `Generate` is clicked, downloads a YAML file whose root `openapi` field is `3.1.2`, that does not use the OpenAPI 3.0-only `nullable: true` construct (OpenAPI 3.1 expresses nullability with a JSON Schema 2020-12 type union or an `anyOf` null branch), and in which the BIE-backed operation contributes a `components/schemas` entry referenced via `#/components/schemas/<BIEName>`.

#### Test Assertion #43.9.2
The same document switched to OpenAPI Version `3.0` and regenerated downloads a YAML file whose root `openapi` field is `3.0.4`, confirming that the generator branches on the document's configured OpenAPI Version.

#### Test Assertion #43.9.3
Because the document is generated from its persisted record, clicking `Generate` while the Edit OpenAPI Document page has unsaved changes (for example, after changing the `OpenAPI Version` without clicking `Update`) is blocked: a notification reads `There are unsaved changes. Please click Update before generating the document.` and no file is downloaded. After clicking `Update`, `Generate` produces a file whose root `openapi` field carries the canonical patch for the newly saved version.

#### Test Assertion #43.9.4
A BBIE that carries a Fixed Value is expressed with the JSON Schema 2020-12 `const` keyword in an OpenAPI `3.1` document — the property emits `const: <value>` and does NOT also emit the OpenAPI 3.0-only single-value `enum` — and is expressed with a single-element `enum: [<value>]` in an OpenAPI `3.0` document, where the property emits exactly that one-element `enum` and does NOT emit the 3.1-only `const` (Issue #1610).
...
2. Open the OpenAPI Document into the Edit OpenAPI Document page and ensure the `OpenAPI Version` is `3.1` (the default for a new document), then click `Update` if the version was changed.
...
5. Verify that the root `openapi` field is `3.1.2`, that the file contains no `nullable: true` construct, and that the BIE-backed operation references a `#/components/schemas/<BIEName>` component schema. (Assertion [#1](#test-assertion-4391))
6. Change the `OpenAPI Version` to `3.0`, click `Update`, then click `Generate` and open the downloaded YAML file.
7. Verify that the root `openapi` field is now `3.0.4`. (Assertion [#2](#test-assertion-4392))
8. Change the `OpenAPI Version` again (for example back to `3.1`) but do NOT click `Update`, then click `Generate`.
9. Verify that generation is blocked, that the notification `There are unsaved changes. Please click Update before generating the document.` is shown, and that no file is downloaded; then click `Update`, click `Generate`, and verify that the downloaded file's root `openapi` field carries the canonical patch for the saved version. (Assertion [#3](#test-assertion-4393))
10. Prepare a BIE whose BBIE carries a Fixed Value (for example, in the BIE editor mark the BBIE used, set its value constraint to `Fixed Value`, enter a fixed value, and restrict the value domain to the `Primitive` `token`). On a document set to OpenAPI Version `3.1` (click `Update` after the version change), assign that BIE, click `Generate`, and locate the fixed-value property in the generated component schema; then on a document at OpenAPI Version `3.0`, assign the same BIE, click `Generate`, and locate the same property.
11. Verify that in the `3.1` document the fixed-value property emits `const: <value>` and no `enum`, and that in the `3.0` document the same property emits a single-element `enum: [<value>]` and no `const`. (Assertion [#4](#test-assertion-4394))

## Test Case 43.10

**DELETE Operation Request Body Across OpenAPI Versions**

Pre-condition: An end-user account that can access the BIE menu exists, a BIE is available to assign to an OpenAPI Document, and the environment allows downloading the generated OpenAPI document file (YAML) for inspection. A `Request` Message Body is selectable for a `DELETE` operation in any OpenAPI Version (only `GET` forbids a `Request` body), but the request body is only honored from OpenAPI Version `3.1`.

### Test Assertion:

#### Test Assertion #43.10.1
In an OpenAPI Document whose OpenAPI Version is `3.1`, a `DELETE` operation with Message Body `Request` shows no "ignored DELETE body" banner (the body is honored) and, after `Generate`, emits a `requestBody` for that operation together with a status-only `202` success response.

#### Test Assertion #43.10.2
In an OpenAPI Document whose OpenAPI Version is `3.0`, the same `DELETE` operation with Message Body `Request` shows an amber banner in Endpoint Details warning that the request body is ignored, and after `Generate` the operation emits NO `requestBody` while the status-only `202` success response remains (the request body is dropped, with no orphan request schema).
...
2. Set the `OpenAPI Version` to `3.1` and click `Update`, then assign the BIE with Verb `DELETE` and Message Body `Request`.
...
5. On a `3.0` document, assign the BIE with Verb `DELETE` and Message Body `Request` and observe the Endpoint Details table.
6. Verify that an amber banner warns that the `Request` body on a `DELETE` operation is ignored in OpenAPI 3.0, then click `Generate` and open the downloaded YAML file.
...
8. On a `3.1` document, assign the BIE with Verb `DELETE` and Message Body `Response`, click `Generate`, and open the downloaded YAML file.
...
10. On a `3.1` document, assign the BIE with Verb `DELETE` and Message Body `Request`, set that operation row's `Error Response` body type to `IETF Problem Details`, click `Update`, then click `Generate` and open the downloaded YAML file and inspect the `DELETE` operation's responses; then switch the same document's `OpenAPI Version` to `3.0`, click `Update`, click `Generate` again, and re-inspect the `DELETE` operation's responses.
11. Verify that in `3.1` the `DELETE` operation declares `415`, `422`, and `500` responses, that the `415` response `$ref`s `#/components/responses/415_UnsupportedMediaType` and the `422` response `$ref`s `#/components/responses/422_UnprocessableContent` (both components are emitted), and that in `3.0` the `415` and `422` responses are gone while the `500` (Internal Server Error) response remains. (Assertion [#4](#test-assertion-43104))
12. On a `3.0` document, assign the BIE with Verb `DELETE` and Message Body `Request`, leave the `Error Response` body type at its default (`No Response Body`), confirm the amber "ignored DELETE body" banner is shown, then click `Generate` and open the downloaded YAML file and inspect the `DELETE` operation.
...
9. On a fresh document, set the OpenAPI Version to `3.1`, click `Update`, add the BIE as `DELETE` + `Request` and then `DELETE` + `Response`, and click `Generate`; then set the OpenAPI Version to `3.0`, click `Update`, inspect the banner, and click `Generate` again; finally set the version back to `3.1`, click `Update`, remove the `Request` row, and click `Generate` once more.
10. Verify that the 3.1 `DELETE` path-item carries a `requestBody` and a `200` response; that in 3.0 the request body is dropped (the amber "ignored DELETE body" banner appears) while the `200` survives; and that after removing the `Request` body the `Response` row remains, the `Request` row is gone, and the `DELETE` operation still generates with the surviving `200` response and no `requestBody`. (Assertion [#5](#test-assertion-43125))
...
**Manage a BIE's OpenAPI Document Bindings from the BIE Root**

Pre-condition: An end-user account that can access the BIE menu exists, and the environment allows downloading the generated OpenAPI document file (YAML) for inspection. The BIE-root editor surfaces a first-class `OpenAPI Document Information` panel that shows the cross-document union of a BIE's OpenAPI Document bindings — one card per binding — and lets the user add, edit, and remove them in place (Issue #1519). The panel surfaces only on the BIE root (level 0) and only when at least one OpenAPI Document exists: it is hidden when no OpenAPI Document exists anywhere, and hidden for a BIE that is neither editable nor bound to any document; an editable BIE with no binding still shows the panel (with an empty-state message and a header `+`) so the first binding can be added from the root. Because every write flows through the SAME backend endpoints as the OpenAPI Document editor — the assign endpoint for the header `+`, the per-document detail-update endpoint for `Update OpenAPI Information`, and the delete endpoint for the per-card `−` — a change made on the BIE root has the identical effect on the database (and therefore on the generated OpenAPI document) as the same change made on the OpenAPI Document screen, and the OpenAPI Document editor reads it back identically. The `Operation ID` is derived the same way on both screens (`<verb-word><BIEName>[List]`, with `GET`→`query`, `POST`→`create`, `PUT`→`replace`, `PATCH`→`update`, `DELETE`→`delete`; Issue #1732). The previously delivered OpenAPI behaviors — OpenAPI 3.1 / `DELETE` body (Issue #1610), per-operation `Error Response Body Type` (Issue #1347), and one Request / one Response per `(Resource Name, Verb)` (Issue #1492) — remain intact when exercised through the new BIE-root edit path. The panel also mirrors the OpenAPI Document editor's read-only `DELETE`-body warning (Issue #1610): a `DELETE` + `Request` binding whose owning document targets OpenAPI `3.0.x` shows an amber per-card warning that the Request body is ignored on generation; because the panel never edits a document's `OpenAPI Version` (that is changed on the OpenAPI Document screen), the warning is informational only and clears once the document is switched to `3.1`. The BIE-root `Add to OpenAPI Document` dialog also applies the same client-side duplicate-body pre-check the OpenAPI Document editor's `Add BIE` dialog applies (Issue #1492): a chosen `(Verb, Message Body)` already present on the selected document is flagged inline and disables `Add` (the backend enforces it as well, returning a 400).

### Test Assertion:

#### Test Assertion #43.14.1
For a BIE that is bound to an OpenAPI Document, the `OpenAPI Document Information` panel surfaces on the BIE root and shows one binding card whose document chip names the bound OpenAPI Document and whose `Verb`, `Message Body`, and derived `Operation ID` match the binding; after `Generate` the downloaded YAML carries that operation's derived `Operation ID` (Issue #1519).

#### Test Assertion #43.14.2
For an editable BIE (WIP, owned) that has no binding yet, the panel still surfaces on the BIE root — provided at least one OpenAPI Document exists — showing the empty-state message `This BIE is not used in any OpenAPI Document yet.` and the header `+` (Add to an OpenAPI Document), so the first binding can be added from the root (Issue #1519).

#### Test Assertion #43.14.3
For a BIE that is neither editable nor bound to any OpenAPI Document, the panel does NOT surface on the BIE root, even though OpenAPI Documents exist. (The panel is also hidden when no OpenAPI Document exists anywhere — a separate `oasDocExists` guard — but that branch is not exercised by this test, because a zero-document state is not deterministically reproducible in the shared e2e database, where the document-list query is global; this case therefore covers only the not-editable-and-unbound branch.) (Issue #1519).

#### Test Assertion #43.14.4
The header `+` opens the `Add to OpenAPI Document` dialog; selecting an OpenAPI Document and a `Verb`/`Message Body` previews the same `Operation ID` the OpenAPI Document editor would derive, and clicking `Add` binds the BIE (snackbar `Added to OpenAPI Document.`), surfaces a new binding card, and — after `Generate` from the OpenAPI Document screen — emits the new operation in the downloaded YAML (Issue #1519).

#### Test Assertion #43.14.5
The per-card `−` unbinds the operation from the OpenAPI Document (snackbar `Removed from OpenAPI Document.`): the binding card disappears and, after `Generate`, the operation no longer appears in the downloaded YAML, while a second operation on the same BIE remains (Issue #1519).

#### Test Assertion #43.14.6
The `Operation ID` is derived identically on the BIE-root and OpenAPI Document screens. For the same BIE, the BIE-root `Add to OpenAPI Document` dialog previews the same `Operation ID` the OpenAPI Document editor derives for every verb — `GET` (`query<BIEName>`), `POST` (`create<BIEName>`), `PUT` (`replace<BIEName>`), `PATCH` (`update<BIEName>`), `DELETE` (`delete<BIEName>`) — and for `POST` + array (`create<BIEName>List`); the BIE-root-created operation then appears with that identical `Operation ID`, verbatim, in the generated document (Issues #1519, #1732).

#### Test Assertion #43.14.7
An edit applied from the BIE root persists and is read back identically by the OpenAPI Document editor: changing a binding's `Verb` to `POST`, making it an array, and setting a `Tag`, then clicking `Update OpenAPI Information` (snackbar `Updated OpenAPI Document information.`), causes the OpenAPI Document editor's Endpoint Details row to show the new `Verb`, the checked array indicator, the recomputed `Operation ID`, and the new `Tag`; `Generate` then reflects the edited `Operation ID` and the `<BIEName>List` array wrapper schema (Issues #1519, #1732).

#### Test Assertion #43.14.8
The same edit-set has an identical effect on the database whether it is applied on the OpenAPI Document screen or on the BIE root. Binding one BIE to two documents as `GET` + `Response`, then applying the identical edit-set (`Verb`→`POST`, array on, a chosen `Operation ID`) to one document on the OpenAPI Document screen and to the other from the BIE root, leaves the two documents' persisted Endpoint Details rows identical (same `Verb`, array indicator, and `Operation ID`), and `Generate` emits the identical operation in both documents (Issue #1519).

#### Test Assertion #43.14.9
Setting a binding's `Error Response` body type to `IETF Problem Details` from the BIE root and clicking `Update OpenAPI Information` keeps Issue #1347 intact: after `Generate`, the document emits the reusable `#/components/responses/500_InternalServerError` response whose `application/problem+json` content references the shared `#/components/schemas/ProblemDetails` schema (Issues #1519, #1347).

#### Test Assertion #43.14.10
Issue #1492 stays enforced on the BIE-root edit path: when a BIE is bound twice to one document (`GET` + `Response` and `POST` + `Response`, two distinct operations / two cards), driving the second card's `Verb` to `GET` so both bindings collide on `(Resource Name, GET, Response)` flags the card with the inline error `Each (Resource Name, Verb) can have only one Request and one Response body.` and disables the `Update OpenAPI Information` button, so the duplicate body slot cannot be saved (Issues #1519, #1492).

#### Test Assertion #43.14.11
Issue #1610 parity, read-only on the BIE screen: a binding card whose `Verb` is `DELETE` and `Message Body` is `Request` shows the amber warning `A Request Body on a DELETE operation is ignored in OpenAPI 3.0. Set this OpenAPI Document's Version to 3.1 on the OpenAPI Document screen to include it in the generated document.` exactly when its owning OpenAPI Document targets OpenAPI `3.0.x` (the body is dropped on generation). The BIE screen never edits the OpenAPI Version itself: after the document's `OpenAPI Version` is switched to `3.1` on the OpenAPI Document screen (where the body is honored), the same card no longer shows the warning (Issues #1519, #1610).

#### Test Assertion #43.14.12
Issue #1492 parity on the BIE-root `Add to OpenAPI Document` dialog: choosing a `Verb` and `Message Body` that would duplicate a body the BIE already has on the SELECTED document (e.g. selecting a document the BIE is already bound to as `GET` + `Response` and choosing `GET` + `Response` again) flags the inline error `This endpoint already has a Response body.` and disables the dialog's `Add` button — the same client-side guard the OpenAPI Document editor's `Add BIE` dialog applies (and the backend enforces it too, returning a 400). Switching the selection to a non-colliding operation (e.g. `POST` + `Response`) clears the error and re-enables `Add` (Issues #1519, #1492).

### Test Step Pre-condition:
1. An end-user account that can access the BIE menu is available in connectCenter.
2. The end user can create top-level BIEs (each under its own Business Context) and OpenAPI Documents.
3. The environment allows downloading the generated OpenAPI document file (YAML) for inspection.

### Test Step:
1. Sign in to connectCenter as the end user. Create a WIP top-level BIE and an OpenAPI Document, open the OpenAPI Document into the Edit OpenAPI Document page, and add the BIE as `GET` + `Response`; click `Generate`. Then open the BIE into the Edit BIE page and inspect the `OpenAPI Document Information` panel.
2. Verify that the panel surfaces on the BIE root with one binding card whose document chip names the OpenAPI Document and whose `Verb` (`GET`), `Message Body` (`Response`), and derived `Operation ID` (`query<BIEName>`) match the binding, and that the generated YAML carries that `Operation ID`. (Assertion [#1](#test-assertion-43141))
3. Open a fresh WIP top-level BIE (with at least one OpenAPI Document existing but the BIE unbound) into the Edit BIE page and inspect the panel.
4. Verify that the panel surfaces with the empty-state message `This BIE is not used in any OpenAPI Document yet.` and the header `+`. (Assertion [#2](#test-assertion-43142))
5. Open a non-editable top-level BIE (e.g. in `Production` state) that has no OpenAPI binding into the Edit BIE page and inspect the BIE root.
6. Verify that the `OpenAPI Document Information` panel does NOT surface. (Assertion [#3](#test-assertion-43143))
7. On a fresh WIP BIE, open the panel's header `+` `Add to OpenAPI Document` dialog, select an OpenAPI Document, set `Verb` `GET` and `Message Body` `Response`, read the `Operation ID` preview, click `Add`, then `Generate` the document from the OpenAPI Document screen.
8. Verify that the dialog previews the same `Operation ID` the OpenAPI Document editor derives, that `Add` binds the BIE (snackbar `Added to OpenAPI Document.`) and surfaces a new card, and that the generated YAML carries the new operation. (Assertion [#4](#test-assertion-43144))
9. With a BIE already bound as `GET` + `Response`, open the BIE root panel, click the binding card's `−`, confirm the removal, then reopen the OpenAPI Document editor and `Generate`.
10. Verify that the card disappears (snackbar `Removed from OpenAPI Document.`) and that the operation no longer appears in the generated YAML, while the other operation on the same BIE remains. (Assertion [#5](#test-assertion-43145))
11. Bind a BIE to one document as `GET` + `Response` on the OpenAPI Document screen and read the row's live-derived `Operation ID` for `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, then `POST` + array. On the BIE root, open the `Add to OpenAPI Document` dialog for a second document and read the `Operation ID` preview for each of `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, and `POST` + array; then complete the `Add` and `Generate` the second document.
12. Verify that the BIE-root previews equal the OpenAPI-screen values for every verb (`query<BIEName>`, `create<BIEName>`, `replace<BIEName>`, `update<BIEName>`, `delete<BIEName>`, and `create<BIEName>List`) and that the BIE-root-created `Operation ID` appears verbatim in the generated document. (Assertion [#6](#test-assertion-43146))
13. With a BIE bound as `GET` + `Response`, open the BIE root panel, change the binding's `Verb` to `POST`, check `Make as an array`, set a `Tag`, click `Update OpenAPI Information`, then reopen the OpenAPI Document editor and `Generate`.
14. Verify that the OpenAPI Document editor's row shows the new `Verb`, the checked array indicator, the recomputed `Operation ID`, and the new `Tag`, and that the generated YAML carries the edited `Operation ID` and a `<BIEName>List` wrapper schema. (Assertion [#7](#test-assertion-43147))
15. Bind one BIE to two documents as `GET` + `Response`. On the OpenAPI Document screen, edit the first document's row (`Verb`→`POST`, array on, a chosen `Operation ID`) and `Update`; from the BIE root, apply the identical edits to the second document's binding and click `Update OpenAPI Information`. Reopen both documents on the OpenAPI Document screen and `Generate` each.
16. Verify that the two documents' persisted rows are identical (same `Verb`, array indicator, and `Operation ID`) and that both generated documents carry the identical operation. (Assertion [#8](#test-assertion-43148))
17. With a BIE bound as `GET` + `Response`, open the BIE root panel, set the binding's `Error Response` to `IETF Problem Details`, click `Update OpenAPI Information`, reopen the OpenAPI Document editor, and `Generate`.
18. Verify that the generated document emits a reusable `#/components/responses/500_InternalServerError` response referencing `#/components/schemas/ProblemDetails`. (Assertion [#9](#test-assertion-43149))
19. Bind a BIE to one document twice — as `GET` + `Response` and as `POST` + `Response` — open the BIE root panel (two cards), and change the `POST` card's `Verb` to `GET`.
20. Verify that the card is flagged with the inline error `Each (Resource Name, Verb) can have only one Request and one Response body.` and that the `Update OpenAPI Information` button is disabled. (Assertion [#10](#test-assertion-431410))
21. On an OpenAPI Document whose `OpenAPI Version` is `3.0`, bind a BIE as `DELETE` + `Request`, then open the BIE into the Edit BIE page and inspect that binding's card in the `OpenAPI Document Information` panel. Then, on the OpenAPI Document screen, set the document's `OpenAPI Version` to `3.1`, click `Update`, reopen the BIE, and re-inspect the card.
22. Verify that, while the document targets `3.0`, the card shows the amber warning `A Request Body on a DELETE operation is ignored in OpenAPI 3.0. Set this OpenAPI Document's Version to 3.1 on the OpenAPI Document screen to include it in the generated document.`, and that after the document is switched to `3.1` (on the OpenAPI Document screen) the card no longer shows the warning. (Assertion [#11](#test-assertion-431411))
