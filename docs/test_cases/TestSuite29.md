...
When a reuse node is mapped during uplift, the reference to the reused BIE must be handled as follows (OAGi/Score#1735):
...
- If the user leaves a mapped reuse node without selecting a BIE to reuse, the system shall warn the user (`Proceed without selecting reuse BIEs?`) and list the affected node path(s) before proceeding. If the user continues, the reuse node's fields are inline-copied into the uplifted BIE and no reference to a reused BIE is kept (the uplifted node shows no reuse icon).
