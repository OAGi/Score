...
DROP TABLE CLASSIFICATION_CTX_SCHEME CASCADE CONSTRAINTS ;
...
COMMENT ON COLUMN AGENCY_ID_LIST.AGENCY_ID
IS
...
CREATE INDEX AGENCY_ID_LIST_AGENCY_ID_IDX ON AGENCY_ID_LIST
(
  AGENCY_ID ASC
) ;
...
LOGGING ;
ALTER TABLE AGENCY_ID_LIST_VALUE ADD CONSTRAINT AGENCY_ID_LIST_VALUE_PK PRIMARY KEY ( AGENCY_ID_LIST_VALUE_ID ) ;
...
COMMENT ON COLUMN ASBIE.BASED_ASCC
IS
'The BASED_ASCC column refers to the ASCC record, which this ASBIE contextualizes.' ;
...
CREATE INDEX ASBIE_BASED_ASCC_IDX ON ASBIE
( BASED_ASCC ASC
) ;
...
(
  ...
  BDT_PRI_RESTRI_ID NUMBER (10) ,
  CODE_LIST_ID      NUMBER (10) ,
  AGENCY_ID_LIST_ID NUMBER (10) ,
  CARDINALITY_MIN   NUMBER (10) NOT NULL ,
...
(
  ...
  DT_SC_PRI_RESTRI_ID NUMBER (10) ,
  CODE_LIST_ID        NUMBER (10) ,
  AGENCY_ID_LIST_ID   NUMBER (10) ,
  MIN_CARDINALITY     NUMBER (10) NOT NULL ,
  MAX_CARDINALITY     NUMBER (10) NOT NULL ,
  DEFAULT_VALUE CLOB ,
  FIXED_VALUE CLOB ,
  DEFINITION CLOB ,
  REMARK                  VARCHAR2 (225 CHAR) ,
...
COMMENT ON COLUMN BBIE_SC.MIN_CARDINALITY
IS
'The minimum occurrence constraint for the BBIE SC. A valid value is 0 or 1.' ;
COMMENT ON COLUMN BBIE_SC.MAX_CARDINALITY
IS
...
(
  ...
  BDT_ID                      NUMBER (10) NOT NULL ,
  CDT_AWD_PRI_XPS_TYPE_MAP_ID NUMBER (10) ,
  CODE_LIST_ID                NUMBER (10) ,
...
(
  ...
  CDT_SC_AWD_PRI_XPS_TYPE_MAP_ID NUMBER (10) ,
  CODE_LIST_ID                   NUMBER (10) ,
  IS_DEFAULT                     CHAR (1) NOT NULL ,
  AGENCY_ID_LIST_ID              NUMBER (10)
)
...
LOGGING ;
ALTER TABLE BIE_USER_EXT_REVISION ADD CONSTRAINT BIE_USER_EXT_REVISION_PK PRIMARY KEY ( BIE_USER_EXT_REVISION_ID ) ;
...
LOGGING ;
ALTER TABLE CDT_AWD_PRI_XPS_TYPE_MAP ADD CONSTRAINT CDT_AWD_PRI_XPS_TYPE_MAP_PK PRIMARY KEY ( CDT_AWD_PRI_XPS_TYPE_MAP_ID ) ;
...
IS
'Internal, primary database key.' ;
COMMENT ON COLUMN CDT_SC_AWD_PRI_XPS_TYPE_MAP.CDT_SC_AWD_PRI
IS
'Foreign key to the CDT_SC_AWD_PRI table.' ;
COMMENT ON COLUMN CDT_SC_AWD_PRI_XPS_TYPE_MAP.XBT_ID
IS
'Foreign key to the XBT table. It identifies an XML schema built-in type that maps to the CDT SC Allowed Primitive identified in the CDT_SC_AWD_PRI column.' ;
...
LOGGING ;
COMMENT ON TABLE CLASSIFICATION_CTX_SCHEME
IS
'This table represents a classification scheme for a context category.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.CLASSIFICATION_CTX_SCHEME_ID
IS
'Internal, primary, database key.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.GUID
IS
'GUID of the classification scheme. Per OAGIS, a GUID is of the form "oagis-id-" followed by a 32 Hex character sequence.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.SCHEME_ID
IS
'External identification of the scheme. ' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.SCHEME_NAME
IS
'Pretty print name of the classification scheme.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.DESCRIPTION
IS
'Description of the classification scheme.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.SCHEME_AGENCY_ID
IS
...
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.SCHEME_VERSION_ID
IS
'Version number of the classification scheme.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.CTX_CATEGORY_ID
IS
...
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.CREATED_BY
IS
'Foreign key to the APP_USER table. It indicates the user who created this classification scheme.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.LAST_UPDATED_BY
IS
'Foreign key to the APP_USER table. It identifies the user who last updated the classification scheme.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.CREATION_TIMESTAMP
IS
'Timestamp when the classification scheme was created.' ;
COMMENT ON COLUMN CLASSIFICATION_CTX_SCHEME.LAST_UPDATE_TIMESTAMP
IS
'Timestamp when the classification scheme was last updated.' ;
CREATE UNIQUE INDEX CTX_SCH_PK_IDX ON CLASSIFICATION_CTX_SCHEME
(
  CLASSIFICATION_CTX_SCHEME_ID ASC
)
LOGGING ;
CREATE INDEX CTX_SCH_CTX_CAT_ID_IDX ON CLASSIFICATION_CTX_SCHEME
(
  CTX_CATEGORY_ID ASC
)
LOGGING ;
CREATE INDEX CTX_SCH_CREATED_BY_IDX ON CLASSIFICATION_CTX_SCHEME
(
  CREATED_BY ASC
)
LOGGING ;
CREATE INDEX CTX_SCH_LAST_UPDATED_BY_IDX ON CLASSIFICATION_CTX_SCHEME
(
  LAST_UPDATED_BY ASC
)
LOGGING ;
ALTER TABLE CLASSIFICATION_CTX_SCHEME ADD CONSTRAINT CTX_SCHEME_PK PRIMARY KEY ( CLASSIFICATION_CTX_SCHEME_ID ) ;
ALTER TABLE CLASSIFICATION_CTX_SCHEME ADD CONSTRAINT CTX_SCHEME_UK1 UNIQUE ( GUID ) ;
...
LOGGING ;
ALTER TABLE CODE_LIST_VALUE ADD CONSTRAINT CODE_LIST_VALUE_PK PRIMARY KEY ( CODE_LIST_VALUE_ID ) ;
...
--  classification scheme in the CLASSIFICATION_CTX_SCHEME table.
...
(
  ...
  MEANING CLOB ,
  OWNER_CTX_SCHEME_ID NUMBER (10)
)
LOGGING ;
COMMENT ON TABLE CTX_SCHEME_VALUE
IS
'This table stores the context scheme values for a particular context classification scheme in the CLASSIFICATION_CTX_SCHEME table.' ;
...
IS
'Foreign key to the CLASSIFICATION_CTX_SCHEME table. It identifies the classification scheme, to which this scheme value belongs.' ;
...
(
  ...
  DEFINITION CLOB ,
  OWNER_DT_ID     NUMBER (10) ,
  MIN_CARDINALITY NUMBER (10) NOT NULL ,
  MAX_CARDINALITY NUMBER (10) ,
  BASED_DT_SC_ID  NUMBER (10)
)
LOGGING ;
...
ALTER TABLE AGENCY_ID_LIST ADD CONSTRAINT AGENCY_ID_LIST_AILV_FK FOREIGN KEY ( AGENCY_ID ) REFERENCES AGENCY_ID_LIST_VALUE ( AGENCY_ID_LIST_VALUE_ID ) NOT DEFERRABLE ;
...
ALTER TABLE ASBIEP ADD CONSTRAINT ASBIEP_BASED_ASCCP_ID FOREIGN KEY ( BASED_ASCCP_ID ) REFERENCES ASCCP ( ASCCP_ID ) NOT DEFERRABLE ;
...
ALTER TABLE ASBIE ADD CONSTRAINT ASBIE_BASED_ASCC_ID_FK FOREIGN KEY ( BASED_ASCC ) REFERENCES ASCC ( ASCC_ID ) NOT DEFERRABLE ;
...
ALTER TABLE BBIE_SC ADD CONSTRAINT BBIE_BBIE_ID_FK FOREIGN KEY ( BBIE_ID ) REFERENCES BBIE ( BBIE_ID ) NOT DEFERRABLE ;
...
ALTER TABLE BIE_USER_EXT_REVISION ADD CONSTRAINT BUER_TOP_LEVEL_ABIE_ID_FK FOREIGN KEY ( TOP_LEVEL_ABIE_ID ) REFERENCES ABIE ( ABIE_ID ) NOT DEFERRABLE ;
...
ALTER TABLE CDT_SC_AWD_PRI ADD CONSTRAINT CDT_PRI_ID_FK FOREIGN KEY ( CDT_PRI_ID ) REFERENCES CDT_PRI ( CDT_PRI_ID ) NOT DEFERRABLE ;

ALTER TABLE CDT_SC_AWD_PRI ADD CONSTRAINT CDT_SC_ID_FK FOREIGN KEY ( CDT_SC_ID ) REFERENCES DT_SC ( DT_SC_ID ) NOT DEFERRABLE ;
...
ALTER TABLE CDT_SC_AWD_PRI_XPS_TYPE_MAP ADD CONSTRAINT CSAPXTM_CSAP_FK FOREIGN KEY ( CDT_SC_AWD_PRI ) REFERENCES CDT_SC_AWD_PRI ( CDT_SC_AWD_PRI_ID ) NOT DEFERRABLE ;
...
ALTER TABLE CTX_SCHEME_VALUE ADD CONSTRAINT CSV_OWNER_CTX_SCHEME_ID_FK FOREIGN KEY ( OWNER_CTX_SCHEME_ID ) REFERENCES CLASSIFICATION_CTX_SCHEME ( CLASSIFICATION_CTX_SCHEME_ID ) NOT DEFERRABLE ;

ALTER TABLE CLASSIFICATION_CTX_SCHEME ADD CONSTRAINT CTX_SCHEME_CREATED_BY_FK FOREIGN KEY ( CREATED_BY ) REFERENCES APP_USER ( APP_USER_ID ) NOT DEFERRABLE ;

ALTER TABLE CLASSIFICATION_CTX_SCHEME ADD CONSTRAINT CTX_SCHEME_CTX_CAT_ID_FK FOREIGN KEY ( CTX_CATEGORY_ID ) REFERENCES CTX_CATEGORY ( CTX_CATEGORY_ID ) NOT DEFERRABLE ;

ALTER TABLE CLASSIFICATION_CTX_SCHEME ADD CONSTRAINT CTX_SCHEME_LAST_UPDATED_BY_FK FOREIGN KEY ( LAST_UPDATED_BY ) REFERENCES APP_USER ( APP_USER_ID ) NOT DEFERRABLE ;
...
CREATE OR REPLACE TRIGGER FKNTM_CLASSIFICATION_CTX_SCHEM BEFORE
UPDATE OF CREATED_BY ON CLASSIFICATION_CTX_SCHEME BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table CLASSIFICATION_CTX_SCHEME is violated');
  END;
/
