...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:svcdoc="http://xmlns.oracle.com/Services/Documentation/V1" xmlns:ebocontext="http://xmlns.oracle.com/EBO/BusinessContext/V1" xmlns="http://www.ascc.net/xml/schematron" exclude-result-prefixes="xsi svcdoc xsd ebocontext">
	...
	<!-- Known Limitation 
		...
		- The XML Schema namespace prefix needs to be 'xsd'.
	-->
	<!-- Version notes.
	...
	<xsl:variable name="xsdURI">http://www.w3.org/2001/XMLSchema</xsl:variable>
	...
	<xsl:template name="schema" match="xsd:schema">
		<xsl:element name="pattern">
			...
			<xsl:element name="rule">
				...
				<xsl:element name="assert">
					<xsl:attribute name="test">count(*[name() != 'xsd:annotation'])= <xsl:value-of select="count(child::*[name() != 'xsd:annotation'])"/></xsl:attribute>#Issue: The '<xsl:value-of select="name(.)"/>' element has unmatched number of child elements except xsd:annotation.
				</xsl:element>
				<xsl:element name="assert">
		...
		<xsl:element name="pattern">
			...
			<xsl:element name="rule">
				...
				<xsl:for-each select=".//xsd:any">
					<xsl:element name="assert">
						<xsl:attribute name="test">count(.//xsd:any) = 1</xsl:attribute>Missing an xsd:any.
					</xsl:element>
					<xsl:if test="count(@id) = 1">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@id = '<xsl:value-of select="@id"/>'</xsl:attribute>xsd:any's id attribute does not match or missing.
						</xsl:element>
					</xsl:if>
					<xsl:if test="count(@namespace) = 1">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@namespace = '<xsl:value-of select="@namespace"/>'</xsl:attribute>xsd:any's namespace attribute does not match or missing.
						</xsl:element>
					</xsl:if>
					<xsl:if test="count(@processContents) = 0">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@processContents = 'strict' or count(@processContents) = 0</xsl:attribute>Missing xsd:any's processContent attribute or the value does not match. Note that the default is 'strict'.
						</xsl:element>
					</xsl:if>
					<xsl:if test="count(@processContents) = 1 and @processContents = 'strict'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@processContents = 'strict' or count(@processContents) = 0</xsl:attribute>Missing xsd:any's processContent attribute or the value does not match. Note that the default is 'strict'.
						</xsl:element>
					</xsl:if>											
					<xsl:if test="count(@processContents) = 1 and @processContents != 'strict'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@processContents = '<xsl:value-of select="@processContents"/>'</xsl:attribute>Missing xsd:any's processContent attribute or the value does not match. 
						</xsl:element>
					</xsl:if>
					<xsl:if test="count(@minOccurs) = 0">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@minOccurs = '1' or count(.//xsd:any/@minOccurs) = 0</xsl:attribute>Missing xsd:any's minOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>			
					<xsl:if test="count(@minOccurs) = 1 and @minOccurs = '1'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@minOccurs = '1' or count(.//xsd:any/@minOccurs) = 0</xsl:attribute>Missing xsd:any's minOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>								
					<xsl:if test="count(@minOccurs) = 1 and @minOccurs != '1'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@minOccurs = '<xsl:value-of select="@minOccurs"/>'</xsl:attribute>Missing xsd:any's minOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>
					<xsl:if test="count(@maxOccurs) = 0">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@maxOccurs = '1' or count(.//xsd:any/@maxOccurs) = 0</xsl:attribute>Missing xsd:any's maxOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>			
					<xsl:if test="count(@maxOccurs) = 1 and @maxOccurs = '1'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@maxOccurs = '1' or count(.//xsd:any/@maxOccurs) = 0</xsl:attribute>Missing xsd:any's maxOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>								
					<xsl:if test="count(@maxOccurs) = 1 and @maxOccurs != '1'">
						<xsl:element name="assert">
							<xsl:attribute name="test">.//xsd:any/@maxOccurs = '<xsl:value-of select="@maxOccurs"/>'</xsl:attribute>Missing xsd:any's minOccurs attribute or the value does not match. 
						</xsl:element>
					</xsl:if>
				</xsl:for-each>
				<xsl:for-each select=".//xsd:element">
