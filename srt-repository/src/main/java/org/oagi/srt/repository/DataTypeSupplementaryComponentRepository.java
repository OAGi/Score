...
public interface DataTypeSupplementaryComponentRepository extends JpaRepository<DataTypeSupplementaryComponent, Long> {
    ...
    public DataTypeSupplementaryComponent findOneByGuidAndOwnerDtId(String guid, long ownerDtId);
    ...
    public DataTypeSupplementaryComponent findOneByOwnerDtIdAndPropertyTermAndRepresentationTerm(
            long ownerDtId, String propertyTerm, String representationTerm
    );

    @Query("select d from DataTypeSupplementaryComponent d where d.ownerDtId = ?1 and d.basedDtScId = ?2")
    public DataTypeSupplementaryComponent findOneByOwnerDtIdAndBasedDtScId(long ownerDtId, long basedDtScId);
}
