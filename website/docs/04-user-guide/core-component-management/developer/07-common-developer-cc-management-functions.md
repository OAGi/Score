...
The section covers the toggling between WIP, Draft, and Candidate CC states. For detailed meaning of these and other states, see [CC States](../02-key-concepts.md#cc-states). The current user has to be the owner of the CC to toggle between these three states. To change the CC state:
...
Purge allows for permanently discarding CCs, Code Lists and Agency ID lists from connectCenter. Such entities along with their logs and comments can never be restored back.

Any developer can purge any CC providing that it is in the Deleted state. See [Delete a newly created CC](#delete-a-newly-created-cc). However, if the CC is still used by another CC (as a property, base, or role of an ASCCP) that is not in the Deleted state, it cannot be purged. For instance, if you want to purge a deleted BCCP which is still used as a property of an ACC. The ACC should be deleted first.
...
The tagging feature in connectCenter allows users to label and categorize core components for better management. With this feature, users can assign custom tags to core components making it easier to locate and work with them later. To label the tag to core components:
...
"Export ASCCPs" allows users to download the schema expression of the selected ASCCP(s) as either XML Schema or JSON Schema. To export ASCCPs:
