...
A published release branch (not the Working branch or a draft release branch) should be selected in order for the end users to manage end-user DTs.
...
See [Create a new DT](../developer/06-dt-management.md#create-a-new-dt); for end users, the plus (+) button is available on a published release branch instead of the Working branch.

## Edit detail of a DT

See [Edit detail of a brand new DT](../developer/06-dt-management.md#edit-detail-of-a-brand-new-dt).

:::info
For the "Namespace" field of an end-user DT, only Non-standard namespaces can be selected.
...
A DT in the Production state can be amended where certain changes can be made.
...
1. [Find a DT](#find-a-dt) in a published release branch.

2. [Open detail page of a DT](#view-detail-of-a-dt) in the Production state.

3. Click the "Amend" button at the top-right corner of the page and confirm in the "Amend this DT?" dialog.
   The DT goes into the WIP state; and its revision number increases by 1.

4. Only the following text fields in the DT detail pane on the right may be updated ("Qualifier", "Six Hexadecimal Identifier", and "Namespace" are locked during an amendment); the "Value Domain" section in the same pane also stays editable (see step 6).
...
6. The end user may want to perform these other actions on the DT:
    ...
    5. [Edit details of an existing SC](../developer/06-dt-management.md#edit-details-of-an-existing-sc).
