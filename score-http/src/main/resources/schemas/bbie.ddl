...
(
    ...
    `cardinality_max`            int(11) DEFAULT NULL COMMENT 'Maximum occurrence constraint of the TO_BBIEP_ID. A valid value is an integer from -1 and up. Specifically, -1 means unbounded. 0 means prohibited or not to use.',
    `facet_min_length`           bigint(20) unsigned DEFAULT NULL COMMENT 'Defines the minimum number of units of length.',
    `facet_max_length`           bigint(20) unsigned DEFAULT NULL COMMENT 'Defines the maximum number of units of length.',
    ...
    `is_nillable`                tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Indicate whether the field can have a null value. This corresponds to the nillable flag in the XML schema.',
    ...
    `is_null`                    tinyint(1) NOT NULL DEFAULT 0 COMMENT 'This column indicates whether the field is fixed to NULL. IS_NULL can be true only if the IS_NILLABLE is true. If IS_NULL is true then the FIX_VALUE and DEFAULT_VALUE columns cannot have a value.',
    `definition`                 text           DEFAULT NULL COMMENT 'Description to override the BCC definition. If NULL, it means that the definition should be inherited from the based BCC.',
    `example`                    text           DEFAULT NULL COMMENT 'An example value for the BBIE.',
    ...
    `last_updated_by`            bigint(20) unsigned NOT NULL COMMENT 'A foreign key referring to the user who has last updated the BBIE record.',
    ...
    `last_update_timestamp`      datetime(6) NOT NULL COMMENT 'The timestamp when the BBIE was last updated.',
