...
public class GenerationContext implements InitializingBean {
    ...
    private List<TopLevelAsbiep> topLevelAsbieps;
    private Map<Long, TopLevelAsbiep> topLevelAsbiepMap;
    ...
    private void init(List<TopLevelAsbiep> topLevelAsbieps) {
        ...
        List<Long> topLevelAsbiepIds = topLevelAsbieps.stream()
                .map(e -> e.getTopLevelAsbiepId()).collect(Collectors.toList());
    ...
    public List<BIE> queryChildBIEs(ABIE abie) {
        ...
        for (BIE bie : sorted(asbieList, bbieList)) {
            if (bie instanceof BBIE) {
                ...
                ABIE roleOfAbie = queryTargetABIE(toAsbiep);
    ...
    public BizCtx findBusinessContext(TopLevelAsbiep topLevelAsbiep) {
        long bizCtxId = (topLevelAsbiep != null) ? bizCtxRepository.findByTopLevelAsbiep(topLevelAsbiep).get(0).getBizCtxId() : 0L;
