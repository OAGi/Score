...
public class GitHubIssueLinkService {
    ...
    /**
     * Reads the linked GitHub issues from the DB cache only (no GitHub calls). Internal cache-read used by
     * {@link #refreshFor} when the viewer is not connected to GitHub; {@code refreshFor} is the single
     * public accessor (it refreshes when connected, and falls back to this cache read otherwise).
     */
    private List<LinkedIssue> listFor(ScoreUser requester, CcType ccType, ManifestId manifestId) {
        assertAuthenticated(requester);
        List<LinkedIssue> result = new ArrayList<>();
        for (GitHubIssueLinkRecord row : getLinkedIssues(query(requester), ccType, manifestId)) {
    ...
    public List<LinkedIssue> refreshFor(ScoreUser requester, CcType ccType, ManifestId manifestId) {
        ...
        List<GitHubIssueLinkRecord> rows = getLinkedIssues(query(requester), ccType, manifestId);
