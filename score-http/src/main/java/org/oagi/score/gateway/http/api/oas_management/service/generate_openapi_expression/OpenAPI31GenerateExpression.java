...
/**
 * Issue #1610: OpenAPI 3.1 / JSON Schema 2020-12 variant of the OAS-document generator.
 * Structurally identical to {@link OpenAPI30GenerateExpression}, but emits 2020-12 schema shapes
 * (type-union nullability instead of {@code nullable:true}, {@code xbt.openApi31Map()}, {@code const}
 * instead of single-value {@code enum}, the {@code examples} array instead of {@code example}) and
 * allows a DELETE operation to carry a request body (permitted in OpenAPI 3.1).
 */
@Component
...
public class OpenAPI31GenerateExpression implements BieGenerateOpenApiExpression, InitializingBean {
    ...
    public OpenAPI31GenerateExpression(GenerationContext generationContext, OpenAPIGenerateExpressionOption option) {
    ...
    public void generateErrorResponses() {
        ...
        // is31=true gates 415/422 onto DELETE (DELETE carries a request body only in 3.1+ — #1610).
        ...
        ErrorResponseSchemas.injectErrorResponses(root, schemas, specs, true);
    }
    ...
    // Backend targets OpenAPI 3.1.x (apiKey | http | oauth2 | openIdConnect).
    ...
    // with no flows yields an empty flows object. Per OpenAPI 3.x each OAuth Flow Object's scopes key
    ...
    private List<Object> buildSecurityRequirements(List<OasSecurityRequirement> source) {
        ...
        // oauth2/openIdConnect carry the scope names; all other types MUST be an empty array (3.x).
    ...
    private class DeleteGeneratorForOperation implements GeneratorForOperation {
        ...
        public void proceed(TopLevelAsbiepSummaryRecord topLevelAsbiep,
                            ...
                            Map<String, Object> path,
                            AsbiepSummaryRecord asbiep) {
            // Issue #1610: OpenAPI 3.1 permits a request body on DELETE.
            //  - A "Request" message body emits the requestBody; its success response is a status-only 202
            //    (Accepted) with NO body -- a DELETE does not echo the request back. This anticipates #1347
            //    (RFC 9457 error responses), whose proposed Verb x Array matrix makes the DELETE success a
            //    202 and carries structured payloads in the 4xx/5xx ProblemDetails responses instead.
            //  - A "Response" message body emits a 200 response carrying the BIE, exactly like the other
            //    body-bearing verbs (POST/PUT/PATCH).
            // Finer control (e.g. a DELETE request body together with a custom success body) is deferred and
            // tracked on #1610.
            if (path != null && path.size() > 0) {
                boolean isArray = template.isArrayForJsonExpression();
                String schemaName = template.getSchemaName();
                ...
                if (template.getMessageBodyType().equals("Request")) {
                    path.put("requestBody", ImmutableMap.<String, Object>builder()
                            ...
                            .put("content", ImmutableMap.<String, Object>builder()
                                    ...
                                    .build())
                            .build());
                    // A DELETE request body is not echoed; pair it with a status-only 202 success.
                    if (!path.containsKey("responses")) {
                        path.put("responses", ImmutableMap.<String, Object>builder()
                                .put("202", ImmutableMap.<String, Object>builder()
                                        .put("description", "")
    ...
    private void fillProperties(Map<String, Object> parent,
        ...
        if (reused) {
            ...
            properties = applyNillableTypeUnion(allOf(properties), isNillable);
        }
    ...
    private void fillProperties(Map<String, Object> parent,
        ...
        // Issue #700 (JSON Schema 2020-12: a single fixed value is expressed with `const`).
        ...
        // Issue #692 (JSON Schema 2020-12: `examples` array instead of the 3.0 singular `example`).
        ...
        if (bbieScList.isEmpty()) {
            ...
            ((Map<String, Object>) properties.get("properties"))
                    .put("content", applyNillableTypeUnion(allOf(contentProperties), isNillable));
    ...
    Map<String, Object> applyNillableTypeUnion(Map<String, Object> properties,
        ...
        if (isNillable) {
            ...
            // Fallback for schemas without explicit 'type' (e.g. allOf/$ref).
            Map<String, Object> prop = new LinkedHashMap();
    ...
    private void fillProperties(Map<String, Object> parent,
        ...
        // Issue #596 (JSON Schema 2020-12: a single fixed value is expressed with `const`).
