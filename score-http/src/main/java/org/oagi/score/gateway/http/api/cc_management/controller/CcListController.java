...
public class CcListController {
    ...
    public PageResponse<CcList> getCcList(
        ...
        request.setComponentType(StringUtils.isEmpty(componentType) ? Collections.emptyList() :
                Arrays.asList(componentType.split(",")).stream().map(e -> e.trim()).filter(e -> !StringUtils.isEmpty(e)).collect(Collectors.toList()));
        request.setDen(den);
