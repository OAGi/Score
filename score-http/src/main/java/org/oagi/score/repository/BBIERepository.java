...
public class BBIERepository implements SrtRepository<BBIE> {
    ...
    public List<BBIE> findByOwnerTopLevelAsbiepIdAndUsed(long ownerTopLevelAsbiepId, boolean used) {
        return findByOwnerTopLevelAsbiepIdsAndUsed(Arrays.asList(ownerTopLevelAsbiepId), used);
    }

    public List<BBIE> findByOwnerTopLevelAsbiepIdsAndUsed(List<Long> ownerTopLevelAsbiepIds, boolean used) {
        return getSelectOnConditionStep()
                .where(and(
