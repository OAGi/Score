...
@pytest.fixture
def created_agency_id_list_manifest_id(token):
    """Get an existing agency ID list manifest ID for testing."""
    # Since we only have read-only tools, we'll use a hardcoded ID
    # In a real scenario, you would create an agency ID list manifest first or use an existing one
    return 1


class TestGetAgencyIdList:
    """Test cases for get_agency_id_list tool."""

    @pytest.mark.asyncio
    async def test_get_agency_id_list_success(self, token, created_release_id):
        """Test successful agency ID list retrieval."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            # First get a list of agency ID lists to find a valid manifest ID
            list_result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
                'limit': 1
            })
            
            # Skip test if no agency ID lists exist
            if not list_result.data.items:
                pytest.skip("No agency ID lists found in the database")
            
            # Use the first agency ID list's manifest ID
            agency_id_list_manifest_id = list_result.data.items[0].agency_id_list_manifest_id
            
            result = await client.call_tool("get_agency_id_list", {
                'agency_id_list_manifest_id': agency_id_list_manifest_id
            })
            
            # Verify the result structure
            assert hasattr(result, 'data')
            assert hasattr(result.data, 'agency_id_list_manifest_id')
            assert result.data.agency_id_list_manifest_id == agency_id_list_manifest_id
            assert hasattr(result.data, 'agency_id_list_id')
            ...
            if result.data.values:
                for value in result.data.values:
                    assert hasattr(value, 'agency_id_list_value_manifest_id')
                    assert hasattr(value, 'agency_id_list_value_id')
                    ...
                    assert hasattr(value, 'is_developer_default')
                    assert hasattr(value, 'is_user_default')

    @pytest.mark.asyncio
    async def test_get_agency_id_list_not_found(self, token):
        """Test agency ID list retrieval with non-existent ID."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_list", {
                    'agency_id_list_manifest_id': 99999
                })

    @pytest.mark.asyncio
    async def test_get_agency_id_list_invalid_id(self, token):
        """Test agency ID list retrieval with invalid ID."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_list", {
                    'agency_id_list_manifest_id': -1
                })


class TestGetAgencyIdLists:
    """Test cases for get_agency_id_lists tool."""

    @pytest.mark.asyncio
    async def test_get_agency_id_lists_success(self, token, created_release_id):
        """Test successful agency ID lists retrieval."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
                'limit': 10
            })
            ...
            if result.data.items:
                ...
                assert hasattr(item, 'agency_id_list_id')
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_with_filters(self, token, created_release_id):
        """Test agency ID lists retrieval with filters."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_with_date_filters(self, token, created_release_id):
        """Test agency ID lists retrieval with date range filters."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_with_ordering(self, token, created_release_id):
        """Test agency ID lists retrieval with ordering."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_invalid_pagination(self, token, created_release_id):
        """Test agency ID lists retrieval with invalid pagination parameters."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_lists", {
                    'release_id': created_release_id,
                    'offset': -1,
                    'limit': 10
                })
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_invalid_limit(self, token, created_release_id):
        """Test agency ID lists retrieval with invalid limit."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_lists", {
                    'release_id': created_release_id,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_invalid_date_range(self, token, created_release_id):
        """Test agency ID lists retrieval with invalid date range format."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_lists", {
                    'release_id': created_release_id,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_invalid_order_by(self, token, created_release_id):
        """Test agency ID lists retrieval with invalid order_by format."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_lists", {
                    'release_id': created_release_id,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_missing_release_id(self, token):
        """Test agency ID lists retrieval without required release_id."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            with pytest.raises(Exception):
                await client.call_tool("get_agency_id_lists", {
                    'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_nonexistent_release(self, token):
        """Test agency ID lists retrieval with non-existent release ID."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': 99999,
                'offset': 0,
...
class TestAgencyIdListAuthentication:
    """Test cases for authentication and authorization."""

    @pytest.mark.asyncio
    async def test_get_agency_id_list_without_auth(self):
        """Test agency ID list retrieval without authentication."""
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_without_auth(self):
        """Test agency ID lists retrieval without authentication."""
        with pytest.raises(Exception):  # Expect HTTP connection to fail
            async with Client("http://localhost:8000/mcp") as client:
                await client.call_tool("get_agency_id_lists", {
                    'release_id': 1,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_list_with_invalid_token(self, invalid_token):
        """Test agency ID list retrieval with invalid token."""
        with pytest.raises(Exception):  # Expect HTTP connection to fail
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_with_invalid_token(self, invalid_token):
        """Test agency ID lists retrieval with invalid token."""
        with pytest.raises(Exception):  # Expect HTTP connection to fail
            async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=invalid_token)) as client:
                await client.call_tool("get_agency_id_lists", {
                    'release_id': 1,
...
class TestAgencyIdListEdgeCases:
    """Test cases for edge cases and boundary conditions."""

    @pytest.mark.asyncio
    async def test_get_agency_id_lists_max_limit(self, token, created_release_id):
        """Test agency ID lists retrieval with maximum limit."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_large_offset(self, token, created_release_id):
        """Test agency ID lists retrieval with large offset."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 1000,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_empty_filters(self, token, created_release_id):
        """Test agency ID lists retrieval with empty filter values."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
    ...
    @pytest.mark.asyncio
    async def test_get_agency_id_lists_special_characters_in_filters(self, token, created_release_id):
        """Test agency ID lists retrieval with special characters in filters."""
        async with Client("http://localhost:8000/mcp", auth=BearerAuth(token=token)) as client:
            result = await client.call_tool("get_agency_id_lists", {
                'release_id': created_release_id,
                'offset': 0,
